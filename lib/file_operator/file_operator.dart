...
// 适用于安卓使用 SAF 的文件操作实现
class AndroidSAFFileOperator extends FileSystemOperator {
  ...
  Future<bool> exist(String uri) async {
    try {
      return await _safUtil.stat(uri, null) != null;
    } catch (e) {
      return false;
    }
  }
  ...
  @override
  Future<bool> mkdir(String uri, String name) async {
    await _safUtil.mkdirp(uri, [name]);
    return true;
  }
  ...
  Future<String?> readAsString(String uri) async {
    ...
    Uint8List uint8list = await _safStream.readFileBytes(uri);
    return utf8.decode(uint8list);
  }

  @override
  Future<String> name(String uri) async {
    SafDocumentFile? safDocumentFile = await _safUtil.stat(uri, null);
    if (safDocumentFile == null) {
      return p.basename(Uri.decodeComponent(uri));
    }
  ...
  Future<bool> isDir(String uri) async {
    var safDocumentFile = await _safUtil.stat(uri, null);
    if (safDocumentFile == null) {
      return false;
    }
  ...
  Future<void> list(
    ...
    var safDocumentFile = await _safUtil.stat(uri, null);
    if (safDocumentFile == null || !safDocumentFile.isDir) {
      return;
    }
    List<SafDocumentFile> safDocumentFileList = await _safUtil.list(uri);
    if (recursive) {
      Queue<String> queue = Queue<String>();
      queue.add(uri);
      while (queue.isNotEmpty) {
        String currentUri = queue.removeFirst();
        var currentDirFiles = await _safUtil.list(currentUri);
        for (var file in currentDirFiles) {
          if (await isDir(file.uri)) {
            queue.add(file.uri);
          } else {
            if (await callBack.call(file.uri)) {
              return;
            }
          }
        }
      ...
      for (var value in safDocumentFileList) {
        if (await callBack.call(value.uri)) {
          return;
        }
      }
    }
  ...
  @override
  Future<String?> pickDirectory(BuildContext context) async {
    var safDocumentFile = await _safUtil.pickDirectory(
  ...
  Future<bool> checkFolderAvailable(String uri) async {
    try {
      return await _safUtil.exists(uri, true) &&
          await _safUtil.hasPersistedPermission(
            uri,
            checkRead: true,
            checkWrite: true,
          );
  ...
  @override
  String join(String uri, String name) {
    return "$uri${Uri.encodeComponent("/$name")}";
  }
  ...
  Future<String> relative(String path, String from) async {
    return p.relative(
      ...
      from: Uri.decodeComponent(from),
    );
  }

  @override
  Future<void> delete(String uri, {bool recursive = false}) async {
    var isDirV = await isDir(uri);
    return await _safUtil.delete(uri, isDirV);
  }

  @override
  Future<void> copyToPath(String uri, String destination) {
    return _safStream.copyToLocalFile(uri, destination);
  }

  @override
  Future<String> dirname(String uri) async {
    final int lastIndex = uri.lastIndexOf('%2');
    if (lastIndex != -1) {
  ...
  Future<Uint8List?> readAsBytes(String uri, {int? start, int? count}) async {
    ...
    return await _safStream.readFileBytes(uri, start: start, count: count);
  }
  ...
  Future<List<String>?> pickFiles(
    ...
    var safDocumentFile = await _safUtil.pickFiles(initialUri: initialUri);
    if (safDocumentFile == null) {
      return null;
    }
    var result = List<String>.empty(growable: true);
    for (var value in safDocumentFile) {
      result.add(value.uri);
    }
    return result;
  }
